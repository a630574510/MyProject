...
{
    public partial class ZX_Storage : BasePage
    {
        ...
        {
            if (!this.DDL_Dealer.SelectedValue.ToString().Equals("-1"))
            {
                List<string> list = new List<string>();
                StringBuilder sb = new StringBuilder();
                sb.Append("<?xml version=\"1.0\" encoding=\"GBK\"?>");
                sb.AppendFormat("<stream><action>DLCDWMLQ</action><userName>ZXXT</userName><hostNo>{0}</hostNo></stream>", this.DDL_Dealer.SelectedValue.ToString());
                string InsertSql = string.Format("insert into ZX_SCF ([action],RequestXml,RequestDate,RequestID,[Status],UpdateSql) values ('DLCDWMLQ','{0}',GETDATE(),'{1}',0,'{2}')", sb.ToString(), CurrentUser.UserId, this.DDL_Dealer.SelectedText.ToString());
                list.Add(InsertSql);
        ...
        protected void Btn_Search_Click(object sender, EventArgs e)
        {
            ...
            if (RoleId == 10)
            {
                sql = string.Format("select * from  ZX_SCF where [action]='DLCDWMLQ' and [Status]=1 and CONVERT(varchar(100), RequestDate, 23) =CONVERT(varchar(100),GETDATE(), 23) and RequestID='{0}'", CurrentUser.UserId);
            }
            else
            {
                sql = "select * from  ZX_SCF where [action]='DLCDWMLQ' and [Status]=1  and CONVERT(varchar(100), RequestDate, 23) =CONVERT(varchar(100),GETDATE(), 23)";
            }
        ...
        private DataTable Correct(DataSet ds)
        {
            ...
            dt.Columns.Add("id");
            dt.Columns.Add("totnum");      //总记录数
            dt.Columns.Add("custName");     //客户名称
            dt.Columns.Add("totList");      //仓库集合
